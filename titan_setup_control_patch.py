...
def register_titan_setup_control(app):
    ...
    def apply_section(st, section, payload):
        ...
        changed = []
        if section in ("ledger_protocol", "ledger", "card_protocol", "all"):
...
(function(){
 ...
 html+=card('Market Protocol Removed','fas fa-store',`<p class="text-[10px] text-[var(--text-muted)] leading-5">ANK, JODI aur PENEL card protocol Setup/Market system se hata diya gaya hai. Ab sirf Ledger tab ke Control me save hoga.</p>`,`<button onclick="try{window.mainNav='ledger';render(true)}catch(e){}" class="mt-3 w-full bg-[var(--primary)] text-white py-3 rounded-xl font-black text-[10px] uppercase">Open Ledger Control</button>`);
