...
def register_titan_realtime_global(app):
    ...
    def titan_realtime_status():
        return jsonify({
            ...
            "version": "2026-07-08-global-realtime-smooth-v2",
            "pollMs": 1800,
            "writeRefresh": True,
            "smoothMode": True,
            "allTabs": True,
    ...
    def titan_realtime_inject(resp):
        try:
            ...
            if not html or "titan-global-realtime-smooth-v2" in html or "</body>" not in html.lower():
                return resp
...
REALTIME_SCRIPT = r'''
<script id="titan-global-realtime-smooth-v2">
(function(){
  if(window.__TITAN_GLOBAL_REALTIME_SMOOTH__) return;
  window.__TITAN_GLOBAL_REALTIME_SMOOTH__ = true;

  const VERSION = '2026-07-08-global-realtime-smooth-v2';
  ...
  ['touchstart','touchmove','wheel','scroll','pointerdown'].forEach(ev=>{
    try { window.addEventListener(ev, touchInteraction, {passive:true, capture:true}); } catch(e) {}
  });
  ...
  window.__TitanRealtime = {version:VERSION, refresh:(r)=>fetchState(r||'manual', true), markWrite, pause:(ms)=>{pausedUntil=now()+Number(ms||1000)}, status:()=>({lastAppliedAt, syncBusy, writeLockUntil, interactionUntil, pollMs:POLL_MS})};
