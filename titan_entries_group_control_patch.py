...
def register_titan_entries_group_control(app):
    ...
    VERSION = "2026-07-10-entries-group-control-v1"
    ...
    def inject_entries_group_control(resp):
        try:
            ...
            if not html or "titan-entries-group-control-v1" in html or "</body>" not in html.lower():
                return resp
...
SCRIPT = r'''
<script id="titan-entries-group-control-v1">
(function(){
 if(window.__TITAN_ENTRIES_GROUP_CONTROL_V1__) return;
 window.__TITAN_ENTRIES_GROUP_CONTROL_V1__=true;
 ...
 function entriesVisible(){
   ...
   if(/ENTRY MANAGEMENT|BULK ENTRY|TODAY ENTRIES/.test(body)) return true;
   const tabs=[...document.querySelectorAll('button,a,div,span')];
   return tabs.some(el=>{const t=(el.textContent||'').trim().toUpperCase();if(t!=='ENTRIES')return false;const r=el.getBoundingClientRect();if(r.top<0||r.bottom<=0||r.top>180)return false;const c=String(el.className||'').toLowerCase();const s=getComputedStyle(el);return /active|selected|current/.test(c)||s.fontWeight==='700'||s.fontWeight==='800'});
 }
 ...
 async function open(){
   try{
     ...
     const m=document.createElement('div');m.id=MOD;m.style.cssText='position:fixed;inset:55px 0 65px;z-index:999999;background:#07111df7;overflow:auto;padding:12px;color:#fff;font-family:Arial';
     const opts=(key)=>g.map(x=>option(x,(s.groupIds?.[key]||[]).includes(x.id))).join('');
     const box=(title,id,key,privateId,privateOn)=>`<div style="background:#132234;border-radius:15px;padding:13px;margin-top:10px"><b>${title}</b><div style="font-size:11px;color:#9db0c0;margin:5px 0 9px">Ek ya multiple WhatsApp groups select karo</div><select id="${id}" multiple size="5" style="width:100%;background:#0e1b29;color:#fff;border:1px solid #ffffff22;border-radius:10px;padding:8px">${opts(key)}</select><label style="display:flex;justify-content:space-between;margin-top:10px;font-size:12px"><span>Private chat allow</span><input id="${privateId}" type="checkbox" ${privateOn?'checked':''}></label></div>`;
     m.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;background:#123047;padding:14px;border-radius:16px"><div><b>📥 ENTRIES GROUP CONTROL</b><div style="font-size:10px;color:#9db0c0;margin-top:4px">Game · Withdrawal · Deposit</div></div><button onclick="document.getElementById('${MOD}').remove()" style="background:#203247;color:#fff;border:0;border-radius:10px;padding:9px 12px">✕</button></div><div style="background:#132234;border-radius:15px;padding:13px;margin-top:10px"><label style="display:flex;justify-content:space-between"><b>Group Routing Enabled</b><input id="egc-enabled" type="checkbox" ${s.enabled!==false?'checked':''}></label><label style="display:flex;justify-content:space-between;margin-top:12px"><span>Only selected groups</span><input id="egc-strict" type="checkbox" ${s.strictGroups===true?'checked':''}></label><div style="font-size:10px;color:#fac748;margin-top:8px">Strict ON karne ke baad selected group ke bahar request reject hogi.</div></div>${box('🎮 Game Entry Group','egc-game','gameEntry','egc-private-game',s.allowPrivate?.gameEntry!==false)}${box('💸 Withdrawal Request Group','egc-withdraw','withdrawal','egc-private-withdraw',s.allowPrivate?.withdrawal!==false)}${box('💳 Deposit Request Group','egc-deposit','deposit','egc-private-deposit',s.allowPrivate?.deposit!==false)}<button id="egc-save" style="width:100%;margin-top:12px;padding:13px;background:#00a884;color:white;border:0;border-radius:12px;font-weight:900">SAVE GROUP ROUTING</button>`;
     document.body.appendChild(m);document.getElementById('egc-save').onclick=save;
   }catch(e){notify('❌ Group Control',String(e.message||e),'danger')}
 }
 function mount(){let b=document.getElementById(BTN);if(!entriesVisible()){if(b)b.remove();document.getElementById(MOD)?.remove();return}if(b)return;b=document.createElement('button');b.id=BTN;b.textContent='👥 GROUP CONTROL';b.style.cssText='position:fixed;right:12px;top:145px;z-index:999998;background:#2563eb;color:#fff;border:0;border-radius:22px;padding:11px 15px;font-weight:900;box-shadow:0 5px 20px #0008';b.onclick=open;document.body.appendChild(b)}
 new MutationObserver(mount).observe(document.documentElement,{childList:true,subtree:true});setInterval(mount,700);setTimeout(mount,250);setTimeout(mount,1400);
})();
</script>
'''
