...
def register_deposit_finance_native(app):
    ...
    try:
        from titan_native_ui import register_titan_native_ui
        register_titan_native_ui(app)
    except Exception:
        pass
    ...
    def titan_deposit_finance_native(resp):
        try:
            ...
            if not html or "finance-deposit-native-v4" in html or "</body>" not in html.lower():
                return resp
...
SCRIPT = r'''
<script id="finance-deposit-native-v4">
(function(){
  ...
  function findFinanceTabs(){const buttons=qa('button,a,div');const hit=buttons.find(b=>{const t=txt(b);return t.includes('SUMMARY')&&t.includes('PAYMENT')})||buttons.find(b=>txt(b).includes('WITHDRAWAL')&&txt(b).includes('PAYMENT'));if(hit)return hit;const pay=buttons.find(b=>txt(b)==='PAYMENT'||txt(b).includes('PAYMENT'));return pay?(pay.parentElement||pay):null}
  function ensureButton(){if(!inFinance())return;if(q('#financeDepositNativeBtn'))return;const wrap=findFinanceTabs();if(!wrap)return;const btn=document.createElement('button');btn.id='financeDepositNativeBtn';btn.type='button';btn.textContent='DEPOSIT';btn.style.cssText='margin:4px;padding:10px 14px;border:0;border-radius:14px;background:#2aabee;color:white;font-weight:900;font-size:11px;letter-spacing:.04em';btn.onclick=clickDeposit;wrap.appendChild(btn)}
  ...
  function show(){ensureButton();if(!inFinance()||!isDeposit()){if(!inFinance())removePanel();return}removePanel();const r=root();if(r)r.insertAdjacentHTML('afterbegin',panelHtml())}
  document.addEventListener('click',function(){setTimeout(show,180)},true);
  const obs=new MutationObserver(function(){ensureButton();if(!inFinance())removePanel()});obs.observe(document.documentElement,{childList:true,subtree:true});
