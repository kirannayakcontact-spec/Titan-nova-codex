...
// User can send only a payment screenshot image. The bot creates a pending
// deposit proof for Finance -> Deposit admin verification. Wallet is never
// credited from screenshot alone.
...
const PATCH_VERSION = "2026-07-08-screenshot-only-deposit-v1";
...
try {
  ...
  if (Array.isArray(raw)) processed = new Set(raw.slice(-1000));
} catch (e) {}

function saveProcessed() {
  try { fs.writeFileSync(PROCESSED_FILE, JSON.stringify(Array.from(processed).slice(-1000))); } catch (e) {}
}
...
function fbChildUrl(parts) {
  const base = FIREBASE_URL.replace(/\.json$/i, "");
  const clean = parts.map(p => encodeURIComponent(String(p))).join("/");
  return `${base}/${clean}.json`;
}
async function fbGet(parts) {
  const axios = require("axios");
  const r = await axios.get(fbChildUrl(parts), { timeout: 12000 });
  return r.data;
}
async function fbPut(parts, value) {
  const axios = require("axios");
  const r = await axios.put(fbChildUrl(parts), value, { timeout: 15000 });
  return r.data;
}
async function fbPatch(parts, value) {
  const axios = require("axios");
  const r = await axios.patch(fbChildUrl(parts), value, { timeout: 15000 });
  return r.data;
}
...
function parseCaption(caption) {
  ...
  const out = { amount: null, utr: "", text };
  const amountMatch = text.match(/(?:₹|rs\.?|inr)?\s*([1-9][0-9]{1,6})(?:\.00)?/i);
  if (amountMatch) out.amount = Number(amountMatch[1]);
  const utrMatch = text.match(/(?:utr|upi|txn|transaction|ref(?:erence)?)[^a-z0-9]{0,12}([a-z0-9]{8,32})/i) || text.match(/\b([0-9]{10,18})\b/);
  if (utrMatch) out.utr = String(utrMatch[1] || "").toUpperCase();
  return out;
}
function likelyDepositCaption(text) {
  const t = String(text || "").toLowerCase();
  return !t || t.includes("deposit") || t.includes("paid") || t.includes("payment") || t.includes("utr") || t.includes("upi") || t.includes("txn") || t.includes("transaction");
}
function riskFor({ amount, utr, duplicate }) {
  const reasons = ["manual_bank_verification_required"];
  if (!amount) reasons.push("amount_missing_from_message");
  if (!utr) reasons.push("utr_missing_from_message");
  if (duplicate) reasons.push("duplicate_screenshot_hash");
  return { level: duplicate ? "high" : (!amount || !utr ? "high" : "medium"), reasons, autoCreditBlocked: true };
}
async function downloadImage(downloadContentFromMessage, imageMessage) {
...
async function safeReply(sock, jid, text, quoted) {
  try { await sock.sendMessage(jid, { text }, quoted ? { quoted } : undefined); } catch (e) {}
}
async function handleDepositScreenshot(sock, upsert, baileys) {
  ...
  for (const m of upsert.messages) {
    try {
      ...
      if (!likelyDepositCaption(caption)) continue;
      const msgKey = `${m.key?.remoteJid || ""}:${m.key?.id || ""}:${m.key?.participant || ""}`;
      ...
      const rec = {
        ...
        status,
        stage: status,
        source: "whatsapp_screenshot_only",
        userId: phone || "guest",
        profileId: phone || "guest",
        phoneNumber: phone,
        ...
        imageHash: imgHash,
        screenshotOnly: true,
        needsManualAmount: !parsed.amount,
        needsManualUtr: !parsed.utr,
        risk,
        duplicateOf: duplicate || null,
        walletCredit: { applied: false, source: PATCH_VERSION, blockedUntilAdminApprove: true },
        createdAt: nowIso(),
        updatedAt: nowIso()
      };
      ...
      console.error("⚠️ Deposit screenshot handler failed:", e && e.message ? e.message : e);
    }
  }
}
    ...
    if (request === "@whiskeysockets/baileys" && exported && !exported.__titanDepositScreenshotWrapped) {
      ...
      if (typeof originalMake === "function") {
        exported.default = function titanDepositMakeWASocketWrapper() {
          ...
          try {
            if (sock && sock.ev && typeof sock.ev.on === "function" && !sock.__titanDepositScreenshotListener) {
              sock.__titanDepositScreenshotListener = true;
              sock.ev.on("messages.upsert", (upsert) => handleDepositScreenshot(sock, upsert, exported));
              console.log("✅ Titan screenshot-only deposit listener active");
            }
          } catch (e) {
            console.error("⚠️ Could not attach deposit screenshot listener:", e && e.message ? e.message : e);
          }
          return sock;
        };
        Object.assign(exported.default, originalMake);
      }
