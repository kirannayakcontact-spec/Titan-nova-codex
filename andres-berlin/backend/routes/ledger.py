...
def register(app):
    ...
    def ledger_entries():
        return jsonify({"entries": list_ledger_entries(request.args.get("account"), int(request.args.get("limit", 100)))})

    @app.post("/api/ledger/entries")
    def ledger_create_entry():
        body = request.get_json(silent=True) or {}
        try:
            entry = create_ledger_entry(body.get("account", ""), body.get("amount"), body.get("type", "credit"), body.get("description", ""), body.get("reference", ""))
