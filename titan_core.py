...
RESULT_SOURCE_NAME = "Dpbosss Net In"
RESULT_SOURCE_URL = "https://dpbosss.net.in/"
...
SAFE_UPDATE_PROTECTED_FEATURES = [
    ...
    {"key": "result_source", "name": "Dpbosss Net In Result Source", "critical": True},
...
CHART_LINKS = [
    {"n": "SRIDEV DAY", "l": "https://dpbosss.net.in/record/sridevi-satta-penal-chart.php"},
    {"n": "TIME BAZAR", "l": "https://dpbosss.net.in/time-bazar-panel-chart.php"},
    {"n": "MADHUR DAY", "l": "https://dpbosss.net.in/madhur-day-panel-chart.php"},
    {"n": "MILAN DAY", "l": "https://dpbosss.net.in/record/milan-day-penal-chart.php"},
    {"n": "RAJDHANI DAY", "l": "https://dpbosss.net.in/record/rajdhani-day-penal-chart.php"},
    {"n": "SUPREME DAY", "l": "https://dpbosss.net.in/supreme-day-panel-chart.php"},
    {"n": "KALYAN", "l": "https://dpbosss.net.in/record/kalyan-penal-chart.php"},
    {"n": "SRIDEVI NIGHT", "l": "https://dpbosss.net.in/record/sridevi-night-satta-penal-chart.php"},
    {"n": "MADHUR NIGHT", "l": "https://dpbosss.net.in/madhur-night-panel-chart.php"},
    {"n": "SUPREME NIGHT", "l": "https://dpbosss.net.in/supreme-night-panel-chart.php"},
    {"n": "MILAN NIGHT", "l": "https://dpbosss.net.in/record/milan-night-penal-chart.php"},
    {"n": "KALYAN NIGHT", "l": "https://dpbosss.net.in/record/kalyan-night-penal-chart.php"},
    {"n": "RAJDHANI NIGHT", "l": "https://dpbosss.net.in/record/rajdhani-night-penal-chart.php"},
    {"n": "MAIN BAZAR", "l": "https://dpbosss.net.in/main-bazar-panel-chart.php"}
]
...
def _market_guess_from_url(url):
    """Best-effort market metadata discovery from a public market/chart URL.

    The registry save path remains manual and deterministic: scraping only fills
    missing name/open/close/chart fields when the admin provides a URL. If a site
    blocks scraping or uses an unknown layout, callers still get a clear error
    asking for manual times instead of saving bad data.
    """
    import re
    ...
    if not parsed.netloc:
        return {}
    meta = {'sourceUrl': raw_url, 'chartUrl': raw_url, 'scrapeStatus': 'url_only'}
    try:
        import requests
        resp = requests.get(raw_url, timeout=10, headers={
            'User-Agent': 'Mozilla/5.0 (compatible; TitanNovaMarketRegistry/1.0)',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
        })
        meta['scrapeHttpStatus'] = int(getattr(resp, 'status_code', 0) or 0)
        text = resp.text or ''
    except Exception as exc:
        meta['scrapeStatus'] = 'fetch_failed'
        meta['scrapeError'] = str(exc)[:180]
        text = ''

    if not text:
        host = parsed.netloc.split(':')[0].replace('www.', '')
        meta['name'] = host.split('.')[0].replace('-', ' ').replace('_', ' ').upper()
        return meta

    flat = re.sub(r'(?is)<(script|style).*?</\1>', ' ', text)
    title_match = re.search(r'(?is)<title[^>]*>(.*?)</title>', flat)
    h1_match = re.search(r'(?is)<h1[^>]*>(.*?)</h1>', flat)
    visible = re.sub(r'(?is)<[^>]+>', ' ', flat)
    visible = _html.unescape(re.sub(r'\s+', ' ', visible)).strip()

    title = ''
    for candidate in (h1_match.group(1) if h1_match else '', title_match.group(1) if title_match else ''):
        candidate = _html.unescape(re.sub(r'(?is)<[^>]+>', ' ', candidate or ''))
        candidate = re.sub(r'\b(chart|result|live|satta|matka|panel|jodi|open|close|today)\b', ' ', candidate, flags=re.I)
        candidate = re.sub(r'[^A-Za-z0-9 ]+', ' ', candidate)
        candidate = re.sub(r'\s+', ' ', candidate).strip()
        if len(candidate) >= 3:
            title = candidate.upper()[:60]
            break
    if title:
        meta['name'] = title

    def norm_time(token):
        m = re.match(r'^(\d{1,2})(?::|\.)(\d{2})\s*([AP]M)?$', str(token or '').strip(), re.I)
        if not m:
            return ''
        h, minute, ap = int(m.group(1)), int(m.group(2)), (m.group(3) or '').upper()
        if ap == 'PM' and h < 12:
            h += 12
        if ap == 'AM' and h == 12:
            h = 0
    ...
    low = visible.lower()
    found = {}
    for label, key in (('open', 'openTime'), ('close', 'closeTime')):
        for m in re.finditer(label, low):
            window = visible[max(0, m.start()-80):m.end()+120]
            tm = re.search(r'(\d{1,2}(?::|\.)\d{2}\s*(?:[AP]M)?)', window, re.I)
            val = norm_time(tm.group(1)) if tm else ''
            if val:
                found[key] = val
                break
    all_times = [norm_time(x) for x in re.findall(r'\b\d{1,2}(?::|\.)\d{2}\s*(?:[AP]M)?\b', visible, re.I)]
    all_times = [x for x in all_times if x]
    if 'openTime' not in found and all_times:
        found['openTime'] = all_times[0]
    if 'closeTime' not in found and len(all_times) > 1:
        found['closeTime'] = all_times[1]
    meta.update(found)
    meta['scrapeStatus'] = 'success' if found or title else 'no_market_metadata_found'
    return meta
...
# Scan Dpbosss Net In live result section, test registry mapping,
...
def api_market_action():
    ...
    try:
        ...
        if action in ('upsert', 'add', 'direct_add', 'direct_add_full'):
            discovered = _market_guess_from_url(payload.get('url') or payload.get('websiteUrl') or payload.get('marketUrl') or payload.get('chartUrl'))
            name = _market_clean_name(payload.get('name') or payload.get('displayName') or discovered.get('name'))
            if not name:
                return jsonify({'status': 'error', 'message': 'Market name ya website URL se name nahi mila', 'marketCoreFix': True}), 400
            open_time = _market_clean_hhmm(payload.get('openTime') or payload.get('open') or discovered.get('openTime'))
            close_time = _market_clean_hhmm(payload.get('closeTime') or payload.get('close') or discovered.get('closeTime'))
            if open_time is None or close_time is None:
                return jsonify({'status': 'error', 'message': 'URL se Open/Close time nahi mila. Manual HH:MM time डालकर save करें.', 'marketCoreFix': True, 'scrape': discovered}), 400
            ...
            item.update({
                ...
                'chartUrl': str(payload.get('chartUrl') or discovered.get('chartUrl') or item.get('chartUrl') or '').strip(),
                'sourceUrl': str(payload.get('url') or payload.get('websiteUrl') or payload.get('marketUrl') or discovered.get('sourceUrl') or item.get('sourceUrl') or '').strip(),
                'scrape': discovered,
                'archived': False,
                'deleted': False,
                'updatedAt': stamp,
...
@app.route('/api/wa_pairing_code', methods=['POST'])
def api_wa_pairing_code():
    try:
        data = request.get_json(silent=True) or {}
        phone = str(data.get('phone') or data.get('mobile') or data.get('number') or '').strip()
        if not phone:
            return jsonify({'status': 'error', 'message': 'Mobile number required'}), 400
        res = _gateway_request('POST', '/wa_pairing_code', json={'phone': phone, 'fresh': True}, timeout=105)
        try:
            payload = res.json()
        except Exception:
            payload = {'status': 'error', 'message': res.text[:300]}
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* ── TITAN NOVA PROFESSIONAL FINANCE UI ── */
        :root {
            --app-bg: #07111D;
            --surface: #0F1E2E;
            --surface-light: #132538;
            --surface-mid: #1A3045;
            --primary: #4ADE80;
            --primary-dark: #22C55E;
            --primary-glow: rgba(74,222,128,.18);
            --green: #4ADE80;
            --green-dark: #22C55E;
            --green-glow: rgba(74,222,128,.16);
            --cyan: #38BDF8;
            --cyan-glow: rgba(56,189,248,.15);
            --rose: #FF6B6B;
            --rose-glow: rgba(255,107,107,.16);
            --amber: #FFD700;
            --text-main: #FFFFFF;
            --text-muted: #A0AEC0;
            --border: #1E293B;
        ...
        body {
            background: radial-gradient(circle at 82% -10%, rgba(74,222,128,.09), transparent 28%), #07111D;
            font-family: 'Inter','Roboto',sans-serif;
        }
        body::before { opacity: .18; }
        .app-bar {
            height: var(--header-h); padding: 0 18px;
            background: rgba(7,17,29,.94); border-bottom: 1px solid var(--border);
            backdrop-filter: blur(18px); box-shadow: 0 8px 30px rgba(0,0,0,.16);
        }
        .brand-mark { width: 38px; height: 38px; display:grid; place-items:center; border-radius:11px; color:#06120b; background:linear-gradient(135deg,#86EFAC,#22C55E); box-shadow:0 0 24px rgba(74,222,128,.22); font-weight:900; }
        .live-chip { display:flex; align-items:center; gap:6px; color:var(--primary); font:700 9px/1 'Inter',sans-serif; letter-spacing:.12em; text-transform:uppercase; }
        .live-dot { width:7px; height:7px; border-radius:50%; background:var(--primary); box-shadow:0 0 0 4px rgba(74,222,128,.12); animation:titanPulse 1.8s infinite; }
        @keyframes titanPulse { 50% { opacity:.45; box-shadow:0 0 0 7px rgba(74,222,128,0); } }
        .header-search { display:none; height:42px; width:min(32vw,380px); align-items:center; gap:10px; padding:0 14px; border:1px solid var(--border); border-radius:10px; background:#0F1E2E; color:var(--text-muted); }
        .header-search input { width:100%; background:transparent; border:0; outline:0; color:#fff; font-size:13px; }
        .header-search input::placeholder { color:#718096; }
        .native-card, .stat-box { box-shadow:0 8px 28px rgba(0,0,0,.14); transition:transform .3s ease,border-color .3s ease,box-shadow .3s ease; }
        .native-card:hover { transform:translateY(-2px); border-color:#2B4055; box-shadow:0 14px 34px rgba(0,0,0,.22); }
        .native-input { min-height:44px; background:#0A1725; border-color:var(--border); }
        button, .nav-item, .side-link-btn { min-height:44px; cursor:pointer; }
        button:focus-visible, input:focus-visible, .nav-item:focus-visible { outline:2px solid var(--primary); outline-offset:2px; }
        .wallet-hud { gap:12px; padding:16px; background:transparent; }
        .stat-box { background:linear-gradient(145deg,#122438,#0D1B2A); border-radius:12px; padding:14px 16px; }
        .stat-val { font-family:ui-monospace,'SFMono-Regular',Consolas,monospace; font-size:17px; letter-spacing:-.02em; }
        .stat-lbl { color:var(--text-muted); letter-spacing:.09em; }
        .pill-tabs { padding:8px 16px; gap:8px; background:transparent; }
        .pill-tab { min-height:42px; border:1px solid transparent; }
        .pill-tab.active { color:#07111D; background:var(--primary); border-color:var(--primary); }
        .bottom-nav { background:rgba(15,30,46,.96); border-color:var(--border); backdrop-filter:blur(18px); }
        .nav-item.active { color:var(--primary); background:rgba(74,222,128,.08); }
        .nav-item.active::after { background:var(--primary); }
        #sidebar { background:#0B1928; border-color:var(--border); }
        .sidebar-grid { display:flex; flex-direction:column; gap:5px; padding:12px; }
        .side-link-btn { display:flex; align-items:center; gap:12px; padding:0 13px; border-radius:10px; text-align:left; font-size:11px; letter-spacing:.04em; }
        .side-link-btn i { width:18px; font-size:14px; text-align:center; }
        .side-link-btn.active { color:var(--primary); background:rgba(74,222,128,.1); border-color:rgba(74,222,128,.24); }
        .sidebar-section-label { padding:14px 13px 5px; color:#64748B; font-size:9px; font-weight:800; letter-spacing:.14em; text-transform:uppercase; }
        .tg-toast { background:#132538; border-color:#29425A; }
        .sheet-content, .install-modal-content { background:#0F1E2E; border-color:var(--primary); }
        .wa-vip-page { background:#07111D; color:#fff; }
        .wa-vip-page .wa-card { background:#0F1E2E; border-color:var(--border); }
        .wa-vip-page .wa-title,.wa-vip-page .wa-name { color:#fff; }
        .wa-vip-page .wa-subtitle,.wa-vip-page .wa-meta,.wa-vip-page .wa-phone { color:var(--text-muted); }
        .wa-vip-page .wa-section { color:var(--amber); }
        .wa-vip-page .wa-input,.wa-vip-page .wa-action,.wa-vip-page input[type=date] { background:#132538; color:#fff; }
        @media (min-width:768px) {
            body { padding-bottom:0; }
            .header-search { display:flex; }
            .bottom-nav { display:none; }
            #sidebar { left:0; width:244px; z-index:60; box-shadow:12px 0 34px rgba(0,0,0,.12); }
            #sidebar .p-5 { min-height:var(--header-h); padding:14px 16px; }
            #top-bar-container { margin-left:244px; }
            #screen-content { margin-left:244px; width:calc(100% - 244px); padding:18px 22px 40px; }
            #sidebar-overlay { display:none !important; }
            .wallet-hud { grid-template-columns:repeat(4,minmax(0,1fr)); }
            .wallet-hud .col-span-2 { grid-column:span 2/span 2; }
            .native-card { margin-bottom:14px; }
        }
        @media (min-width:1024px) {
            #screen-content { padding:24px 30px 48px; }
            .wallet-hud { padding:0 0 18px; }
            .stat-box { padding:18px; }
            .stat-val { font-size:20px; }
        }
        @media (min-width:1440px) { #screen-content { max-width:1500px; } }
    ...
    <script>
        ...
        function titanApplyStartupToolRoute(){
            try{
                ...
                if(requestedTab) mainNav = ['ledger','entries','markets','forward','setup','smart'].includes(requestedTab) ? 'finance' : requestedTab;
        ...
        async function addMarketFromManager(){
            ...
            if(!String(url).trim() && !String(name).trim()) return showRealNotification('⚠️ Market URL','Market website URL paste karo, ya manual market name do.','warning');
            if(!String(url).trim() && !openTime && !closeTime) return showRealNotification('⚠️ Market Time','Manual add me Open ya Close me se ek valid HH:MM time do. URL add me system scrape karega.','warning');
            const data = await marketAction('direct_add_full', {url, name, websiteName, openTime, closeTime, chartUrl,
        ...
        window.addEventListener("popstate", function(e) {
            ...
            else if (mainNav !== 'finance') { setMainNav('finance'); }
        });
                ...
                try {
                    ...
                    if(newState.profiles && newState.profiles[appState.activeId]) {
                        ...
                        if (currentLocalStateStr !== fetchedStateStr || currentAccess !== fetchedAccess) {
                            ...
                            let isSafeTab = (mainNav === 'finance' || mainNav === 'results' || mainNav === 'audit' || mainNav === 'guard' || mainNav === 'health');
        ...
        function withdrawalUpiId(w){
            const direct = String((w || {}).extractedUpiVpa || '').trim();
        ...
        function withdrawalUpiPayLink(w){
            ...
            const name = String((w || {}).userName || (w || {}).userId || 'Titan User');
            return 'upi://pay?pa=' + encodeURIComponent(upi) + '&pn=' + encodeURIComponent(name) + '&am=' + encodeURIComponent(amount.toFixed(2)) + '&cu=INR&tn=' + encodeURIComponent(note);
        }
        ...
        function renderWithdrawalButtons(w){
            ...
            if(status === 'approved'){
                return `<div class="grid grid-cols-3 gap-2 mt-3"><button onclick="openWithdrawalPayNow('${id}')" class="bg-[var(--primary)] text-white py-3 rounded-xl font-black text-[10px] uppercase active:scale-95"><i class="fas fa-mobile-screen-button mr-1"></i> Pay via UPI App</button><button onclick="markWithdrawalPaid('${id}')" class="bg-[var(--green)] text-white py-3 rounded-xl font-black text-[10px] uppercase active:scale-95"><i class="fas fa-circle-check mr-1"></i> Mark Paid</button><button onclick="withdrawalAction('${id}','reject')" class="bg-[rgba(255,93,93,0.12)] text-[var(--rose)] border border-[rgba(255,93,93,0.25)] py-3 rounded-xl font-black text-[10px] uppercase active:scale-95"><i class="fas fa-ban mr-1"></i> Cancel</button></div>`;
            }
...
withdraw status</pre>
            ...
            if(!rows.length){
                ...
                html += `<div class="space-y-2">${rows.map(w=>{
                    ...
                    return `<div class="native-card p-3">
                        ...
                        ${detail}
                        <div class="grid grid-cols-2 gap-2 mt-3"><button onclick="copyWithdrawalDetail('${attrEscape(w.id)}')" class="bg-[var(--surface-light)] text-white border border-[var(--border)] py-2 rounded-xl font-black text-[9px] uppercase active:scale-95"><i class="fas fa-copy mr-1"></i> Copy Details</button>${withdrawalUpiPayLink(w) ? `<button onclick="openWithdrawalPayNow('${attrEscape(w.id)}')" class="text-center bg-[rgba(42,171,238,0.12)] text-[var(--primary)] border border-[rgba(42,171,238,0.25)] py-2 rounded-xl font-black text-[9px] uppercase active:scale-95"><i class="fas fa-mobile-screen mr-1"></i> Pay UPI</button>` : `<span></span>`}</div>
        ...
        async function requestWhatsAppPairingCode(){
            const input = document.getElementById('wa-pairing-phone');
            const phone = (input && input.value || '').trim();
            const box = document.getElementById('wa-pairing-result');
            if(!phone){
                showRealNotification('❌ Mobile Required', 'WhatsApp mobile number enter karo.', 'danger');
                if(input) input.focus();
                return;
            }
            if(box) box.innerHTML = '<span class="text-[var(--amber)]">Pairing code generate ho raha hai...</span>';
            try{
                const res = await fetch('/api/wa_pairing_code', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({phone})});
                const data = await res.json();
                appState.waPairingResult = data;
                if(data.status !== 'success' && data.status !== 'already_connected') throw new Error(data.message || 'Pairing code failed');
                if(box) box.innerHTML = pairingCodeHtml(data);
                showRealNotification('✅ Link Code Ready', data.pairingCode ? ('Code: ' + data.pairingCode) : (data.message || 'WhatsApp already connected'), 'success');
                setTimeout(refreshHealthMonitor, 2500);
            }catch(e){
                appState.waPairingResult = {status:'error', message:String(e.message || e)};
                if(box) box.innerHTML = pairingCodeHtml(appState.waPairingResult);
                showRealNotification('❌ Link Code Error', String(e.message || e), 'danger');
            }
        }
        function pairingCodeHtml(data){
            data = data || {};
            if(data.status === 'already_connected') return '<span class="text-[var(--green)] font-bold">WhatsApp already connected.</span>';
            if(data.pairingCode){
                return '<div class="bg-[rgba(0,194,111,0.08)] border border-[rgba(0,194,111,0.16)] rounded-xl p-3 text-center"><p class="text-[9px] text-[var(--text-muted)] uppercase font-black">Mobile Link Code</p><p class="text-white text-[24px] font-black tracking-[0.18em] mt-1">' + data.pairingCode + '</p><p class="text-[10px] text-[var(--text-muted)] mt-2">WhatsApp → Linked devices → Link with phone number me ye code enter karo. QR scan ya secondary phone ki zarurat nahi.</p></div>';
            }
            if(data.message){
                const safe = String(data.message).replace(/[<>&]/g, function(s){ return ({'<':'&lt;','>':'&gt;','&':'&amp;'}[s]); });
                return '<span class="text-[var(--rose)]">' + safe + '</span>';
            }
            return '<span class="text-[var(--text-muted)]">Mobile number daal ke Get Link Code dabao.</span>';
        }
                    ...
                    <div class="native-card p-4 mb-3 border border-[rgba(42,171,238,0.18)]">
                        ...
                        ${waConnected ? `<div class="bg-[rgba(0,194,111,0.08)] border border-[rgba(0,194,111,0.16)] rounded-xl p-3 text-[10px] text-[var(--green)]">WhatsApp connected: ${gw.user ? (gw.user.name || gw.user.id || 'Connected') : 'Connected'}</div>` : `
                            <div class="grid grid-cols-1 gap-3">
                                ...
                                <div class="bg-[rgba(42,171,238,0.07)] border border-[rgba(42,171,238,0.16)] rounded-xl p-3">
                                    <p class="text-white font-black text-[11px] uppercase mb-2"><i class="fas fa-mobile-screen-button text-[var(--primary)] mr-1"></i> Mobile Number Link</p>
                                    <div class="grid grid-cols-[1fr_auto] gap-2">
                                        <input id="wa-pairing-phone" class="native-input text-xs" inputmode="tel" autocomplete="tel" placeholder="Same WhatsApp number, e.g. 919876543210">
                                        <button onclick="requestWhatsAppPairingCode()" class="bg-[var(--primary)] text-white px-3 rounded-xl font-black text-[9px] uppercase active:scale-95">Get Code</button>
                                    </div>
                                    <p class="text-[9px] text-[var(--text-muted)] mt-2">Primary WhatsApp account ka same mobile number country code ke saath daalo. Galat/other number se code reject hoga.</p>
                                    <div id="wa-pairing-result" class="mt-2 text-[10px]">${pairingCodeHtml(appState.waPairingResult || waLogin)}</div>
                                </div>
                            </div>`}
                        <div class="grid grid-cols-2 gap-2 mt-3">
        ...
        function renderAppBar() {
            ...
            let titleSection = (appState.activeId === 'admin1')
                ? `<div class="flex items-center gap-3"><span class="brand-mark">T</span><div><h1 class="text-[15px] font-black tracking-tight text-white leading-tight">TITAN NOVA</h1><span class="live-chip mt-1"><span class="live-dot"></span>Markets live</span></div></div>`
                : `<div class="flex items-center gap-3"><span class="brand-mark"><i class="fas fa-crown text-xs"></i></span><div><h1 class="text-[15px] font-black tracking-tight text-white leading-tight">${state.name}</h1><span class="live-chip mt-1"><span class="live-dot"></span>VIP secure</span></div></div>`;

            const searchSection = `<label class="header-search" aria-label="Search dashboard"><i class="fas fa-search text-xs"></i><input id="titan-global-search" type="search" placeholder="Search users, markets, transactions..." onkeydown="handleGlobalSearch(event)"></label>`;
            ...
            container.innerHTML = `${leftIcon} ${titleSection} ${searchSection} ${rightSection}`;
        }
        ...
        function handleGlobalSearch(event) {
            ...
            } : { dashboard:'membership', profile:'membership', wallet:'membership', vip:'membership' };
            const match = Object.keys(routes).find(key => query.includes(key));
            if(match) {
                setMainNav(routes[match]);
                event.target.value = '';
                showRealNotification('Search', `Opened ${match}`, 'success');
            } else {
                showRealNotification('No match', 'Try users, markets, wallet, WhatsApp, security or settings.', 'info');
            }
        }
        ...
        function renderBottomNav() {
            ...
            navItems.forEach(item => {
                navHtml += `<div role="button" tabindex="0" onclick="setMainNav('${item.id}')" onkeydown="if(event.key==='Enter'||event.key===' ')setMainNav('${item.id}')" class="nav-item ${mainNav === item.id ? 'active' : ''}"><i class="fas ${item.icon}"></i><span>${item.label}</span></div>`;
            });
            ...
            if(sidebarNav) {
                ...
                const resources = '';
                sidebarNav.innerHTML = `<div class="sidebar-section-label">Workspace</div>${management}`;
            }
        ...
        function renderSubTabs() {
            ...
            return `
                <div class="pill-tabs">
                    ...
                    <button onclick="setSubTab('pannel')" class="pill-tab ${t === 'pannel' ? 'active' : ''}">Panel <span class="text-[9px] opacity-60 ml-1">x150</span></button>
                </div>
            `;
        }
            ...
            return `
                <div class="px-3 py-4">
                    ...
                    <div class="native-card p-4 mb-6 space-y-2">
                        ...
                        ${protoRow('pannel', 'PANEL (x150)','text-[var(--amber)]')}
                    </div>
                ...
                <div class="native-card p-4 mb-3" style="border-color:rgba(250,199,72,0.22);background:rgba(250,199,72,0.04)">
                    ...
                    <div class="mt-3 flex items-center justify-between gap-3 bg-[var(--surface-light)] border border-[var(--border)] rounded-xl p-3">
                        <div class="min-w-0">
                            ...
                            <p class="text-[9px] text-[var(--text-muted)] leading-relaxed">${autoScrapeOn ? 'ON: Gateway Dpbosss Net In live page se result detect karega.' : 'OFF: Gateway scrape skip karega. Manual Declare active hai.'}</p>
                            <p class="text-[8px] text-[var(--text-muted)] mt-1 break-all">Source: ${(appState.resultSettings && appState.resultSettings.sourceUrl) || 'https://dpbosss.net.in/'}</p>
                        </div>
        ...
        function renderAdminPaymentsTab() {
            ...
            return `<div class="px-3 py-4">
                ...
                <div class="native-card p-4 mb-3" style="border-color:rgba(0,194,111,0.2); background:rgba(0,194,111,0.04)">
                    ...
                    <div class="grid grid-cols-2 gap-2"><label id="pm-qr-label" for="pm-qr-input" class="text-center bg-[var(--surface-light)] border border-[var(--border)] text-white py-3 rounded-xl font-black text-[10px] uppercase active:scale-95">${pm.qr ? 'QR Uploaded ✓' : 'Upload QR'}</label><button onclick="savePaymentMethods()" class="bg-[var(--green)] text-white py-3 rounded-xl font-black text-[10px] uppercase active:scale-95">Save</button></div>
                    <img id="pm-qr-preview" src="${pm.qr || ''}" class="${pm.qr ? '' : 'hidden'} mt-3 w-32 h-32 object-contain rounded-xl border border-[var(--border)] bg-white p-1" alt="Payment QR preview">
                </div>
